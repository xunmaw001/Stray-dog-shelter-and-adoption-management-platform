            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑在库犬</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">在库犬信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>狗性别</label>
                                        <select id="sexdogTypesSelect" name="sexdogTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>品种</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input name="file" type="file" id="zaikuquanPhotoupload"
                                               class="form-control-file">
                                        <input name="zaikuquanPhoto" id="zaikuquanPhoto-input" type="hidden">
                                    </div>
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>犬类型</label>
                                        <select id="zaikuquanTypesSelect" name="zaikuquanTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>详情信息</label>
                                        <input id="zaikuquanContentupload" name="file" type="file">
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#zaikuquanPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('zaikuquanPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('zaikuquanPhoto-input').setAttribute('value',photoUrl);
            }
        });


        $('#zaikuquanContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('zaikuquanContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("zaikuquan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addzaikuquan');
    ...
    // 查询列表
        ...
        function zaikuquanTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zaikuquan_types", "GET", {}, (res) => {
                if(res.code == 0){
                    zaikuquanTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationZaikuquantypesSelect(){
            var zaikuquanTypesSelect = document.getElementById('zaikuquanTypesSelect');
            if(zaikuquanTypesSelect != null && zaikuquanTypesOptions != null  && zaikuquanTypesOptions.length > 0 ){
                for (var i = 0; i < zaikuquanTypesOptions.length; i++) {
                    zaikuquanTypesSelect.add(new Option(zaikuquanTypesOptions[i].indexName,zaikuquanTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(sexdogTypesSelect != null && sexdogTypesOptions != null  && sexdogTypesOptions.length > 0 ) {
            for (var i = 0; i < sexdogTypesOptions.length; i++) {
                if (sexdogTypesOptions[i].codeIndex == ruleForm.sexdogTypes) {//下拉框value对比,如果一致就赋值汉字
                        sexdogTypesSelect.options[i].selected = true;
                }
            }
        }
        ...
        if(zaikuquanTypesSelect != null && zaikuquanTypesOptions != null  && zaikuquanTypesOptions.length > 0 ) {
            for (var i = 0; i < zaikuquanTypesOptions.length; i++) {
                if (zaikuquanTypesOptions[i].codeIndex == ruleForm.zaikuquanTypes) {//下拉框value对比,如果一致就赋值汉字
                        zaikuquanTypesSelect.options[i].selected = true;
                }
            }
        }
    }
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.zaikuquanContent != null && ruleForm.zaikuquanContent != 'null' && ruleForm.zaikuquanContent != '' && $("#zaikuquanContentupload").length>0) {
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#zaikuquanContentupload").length>0) {
            var zaikuquanContentEditor = UE.getEditor('zaikuquanContentEditor');
            if (zaikuquanContentEditor.hasContents()) {
                $('#zaikuquanContent-input').attr('value', zaikuquanContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function jieshouTimeChickValue(e){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                zaikuquanName: "required",
                sexdogTypes: "required",
                zaikuquanPinzhong: "required",
                zaikuquanXingge: "required",
                zaikuquanPhoto: "required",
                jieshouTime: "required",
                sfyimiaoTypes: "required",
                sfjueyuTypes: "required",
                zaikuquanTypes: "required",
                zaikuquanContent: "required",
            },
            messages: {
                zaikuquanName: "狗编号不能为空",
                sexdogTypes: "狗性别不能为空",
                zaikuquanPinzhong: "品种不能为空",
                zaikuquanXingge: "性格不能为空",
                zaikuquanPhoto: "生活照片不能为空",
                jieshouTime: "接收时间不能为空",
                sfyimiaoTypes: "是否打过疫苗不能为空",
                sfjueyuTypes: "是否绝育不能为空",
                zaikuquanTypes: "犬类型不能为空",
                zaikuquanContent: "详情信息不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addzaikuquan = window.sessionStorage.getItem("addzaikuquan");
        if (addzaikuquan != null && addzaikuquan != "" && addzaikuquan != "null") {
            window.sessionStorage.removeItem('addzaikuquan');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("zaikuquan/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
